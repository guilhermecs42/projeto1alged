...
bool sistema_sair(REGISTRO** registro_de_pacientes, char* nome_arquivo, FILA** fila_de_espera, char* arquivo_fila){ // GIOVANNI: INCLUIR A FILA DE ESPERA NOS PARÂMETROS
...
void registrar_paciente(REGISTRO* registro_de_pacientes, FILA* fila_de_espera){
	...
	if(!fila_buscar(fila_de_espera, paciente_inserido)){
		fila_inserir(fila_de_espera, paciente_inserido);
	}
	return;
}

void chamar_paciente(FILA* fila_de_espera){
...
void mostrar_fila(FILA* fila_de_espera){
...
void paciente_morreu(REGISTRO* registro_de_pacientes, FILA* fila_de_espera){ // GIOVANNI: ADICIONE O PARÂMETRO FILA* fila_de_espera AQUI
	...
	if(perguntar_numero(&id_lido)==false){
	 ...
	 if(fila_buscar(fila_de_espera, id_lido)){
...
int main(){
	...
	while(1){
		...
		if(strcmp(arquivo_fila, "1")==0){
			...
			fila_de_espera = fila_criar(get_paciente_funcoes());
			if(fila_de_espera==NULL){
				printf("Falha ao criar a Fila de Espera. Tente novamente.");
				continue;
			}
			break;
		}
